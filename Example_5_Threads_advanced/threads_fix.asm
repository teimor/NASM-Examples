...
memory_fill_si:
				...
				mov		r15, 0					; set first index in counter
				mov		r14, [SIZE_ARRAY]			; save back the start index to r14
...
memory_fill_si_lp:
				...
				jb memory_fill_si_lp				; if below, do continue
...
calc_thread:
				...
				je c_first_thread					; handle the first thread
...
c_first_thread:	mov		rax, 0						; set rax as zero
				jmp c_other_threads
...
calc_thread_lp:
				...
				movlps	xmm0, qword [rax]			; get value from memory
				
				; Calc the sum & sum^2
				movq	xmm1, xmm0					; copy the value for sum
				movq	xmm2, xmm0					; copy the value for sum^2
				...
				inc		r15							; increase counter				
				cmp 	r15, r14					; compare to last index for the thread
				jb calc_thread_lp					; if below, do continue				
...
create_thread2_lp:
					...
					mov 	rdi, rax					; pass memory index for this thread
					xor 	rsi, rsi					; clear rsi
					mov 	rdx, calc_thread			; set function to run in thread
					mov 	rcx, r11					; set pointer to save thread_id in array
					call pthread_create					; call pthread_create
					...
					mov 	r10, [THREAD_CNT]			; mov to r10 for compare
					cmp 	r10, [N_THREADS]			; mov number of threads for compare
					jb create_thread2_lp				; if equal return, else continue
					pop rbp								; clear stock
					ret									; return
